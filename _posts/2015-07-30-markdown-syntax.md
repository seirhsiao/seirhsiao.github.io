...
layout: post
title: markdown 语法
description: Markdown(markdown-syntax-zhtw) 语法。
keywords: Markdown
categories: markdown
...
Markdown Syntax, translate from Triditional Chinese Edition 
Document of Markdown Syntax [Markdown 語法說明中文版][markdown-syntax-zhtw]. 
If you are seeking for English Edition 
...
Markdown 不是要来取代 HTML，甚至也没有要和它相似，它的语法种类不多，只和 HTML 的一部分有关係，重点*不是*要创造一种更容易写作 HTML 文件的语法，我认为 HTML 已经很容易写了，Markdown 的重点在於，它能让文件更容易阅读、编写。HTML 是一种*发佈*的格式，Markdown 是一种*编写*的格式，因此，Markdown 的格式语法只涵盖纯文字可以涵盖的範围。

不在 Markdown 涵盖範围之外的标籤，都可以直接在文件里面用 HTML 撰写。不需要额外标註这是 HTML 或是 Markdown；只要直接加标籤就可以了。

只有区块元素──比如 `<div>`、`<table>`、`<pre>`、`<p>` 等标籤，必须在前後加上空行，以利与内容区隔。而且这些（元素）的开始与结尾标籤，不可以用 tab 或是空白来缩排。Markdown 的产生器有智慧型判断，可以避免在区块标籤前後加上没有必要的 `<p>` 标籤。
...
HTML 的区段标籤如 `<span>`、`<cite>`、`<del>` 则不受限制，可以在 Markdown 的段落、清单或是标题里任意使用。依照个人习惯，甚至可以不用Markdown 格式，而采用 HTML 标籤来格式化。举例说明：如果比较喜欢 HTML 的  `<a>` 或 `<img>` 标籤，可以直接使用这些标籤，而不用 Markdown 提供的连结或是影像标示语法。

HTML 区段标籤和区块标籤不同，在区段标籤的範围内， Markdown 的语法是有效的。
...
才能放到连结标籤的 `href` 属性里。不用说也知道这很容易忘记，这也可能是 HTML 标準检查所检查到的错误中，数量最多的。
...
不过需要注意的是，code 範围内，不论是行内还是区块， `<` 和 `&` 两个符号都*一定*会被转换成 HTML 实体，这项特性让你可以很容易地用 Markdown 写 HTML code （和 HTML 相对而言， HTML 语法中，你要把所有的 `<` 和 `&` 都转换为 HTML 实体，才能在 HTML 文件里面写出 HTML code。）
...
如果你*真的*想要插入 `<br />` 标籤的话，在行尾加上两个以上的空白，然後按 enter。
...
Markdown 使用 email 形式的区块引言，如果你很熟悉如何在 email 信件中引言，你就知道怎麽在 Markdown 文件中建立一个区块引言，那会看起来像是你强迫断行，然後在每行的最前面加上 `>` ：
...
任何标準的文字编辑器都能简单地建立 email 样式的引言，例如 BBEdit ，你可以选取文字後然後从选单中选择*增加引言阶层*。
...
清单项目标记通常是放在最左边，但是其实也可以缩排，最多叁个空白，项目标记後面则一定要接著至少一个空白或 tab。
...
在程式码区块里面， `&` 、 `<` 和 `>` 会自动转成 HTML 实体，这样的方式让你非常容易使用 Markdown 插入範例用的 HTML 原始码，只需要複製贴上，再加上缩排就可以了，剩下的 Markdown 都会帮你处理，例如：
...
你可以在一行中用叁个或以上的星号、减号、底线来建立一个分隔线，行内不能有其他东西。你也可以在星号中间插入空白。下面每种写法都可以建立分隔线：
...
要建立一个行内形式的连结，只要在方块括号後面马上接著括号并插入网址连结即可，如果你还想要加上连结的 title 文字，只要在网址後面，用双引号把 title 文字包起来即可，例如：
...
参考形式的连结使用另外一个方括号接在连结文字的括号後面，而在第二个方括号里面要填入用以辨识连结的标籤：
...
*预设的连结标籤*功能让你可以省略指定连结标籤，这种情形下，连结标籤和连结文字会视为相同，要用预设连结标籤只要在连结文字後面加上一个空的方括号，如果你要让 "Google" 连结到 google.com，你可以简化成：
...
连结的定义可以放在文件中的任何一个地方，我比较偏好直接放在连结出现段落的後面，你也可以把它放在文件最後面，就像是註解一样。
...
参考式的连结其实重点不在於它比较好写，而是它比较好读，比较一下上面的範例，使用参考式的文章本身只有 81 个字元，但是用行内形式的连结却会增加到 176 个字元，如果是用纯 HTML 格式来写，会有 234 个字元，在 HTML 格式中，标籤比文字还要多。

使用 Markdown 的参考式连结，可以让文件更像是浏览器最後产生的结果，让你可以把一些标记相关的资讯移到段落文字之外，你就可以增加连结而不让文章的阅读感觉被打断。
...
程式码区段的起始和结束端都可以放入一个空白，起始端後面一个，结束端前面一个，这样你就可以在区段的一开始就插入反引号：
...
*   一个惊叹号 `!`
*   接著一对方括号，里面放上图片的替代文字
*   接著一对普通括号，里面放上图片的网址，最後还可以用引号包住并加上
    选择性的 'title' 文字。
...
[othree]:https://github.com/othree/markdown-syntax-zhtw
